    ...
    return (
        <div className="m-2">
            <div className="flex flex-col xl:flex-row gap-2">
                <button
                    className={`xl:w-1/2 row-start-1 row-end-2  min-h-10 rounded-xl ${isOpen && typeModalWindow === MODAL_TYPE.ADD_CLIENT ? `${THEME_COLORS[currentThemeApp].accentColor}` : tempClientName.length > 0 ? 'bg-emerald-500 hover:bg-emerald-200' : `${THEME_COLORS[currentThemeApp].container.main}`}  flex items-center justify-center`}
                    onClick={() => (
                        handleOpenModal(),
